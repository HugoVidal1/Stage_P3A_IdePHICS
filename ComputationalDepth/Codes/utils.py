...
def quadratic_kernel(x1, x2):
    return (torch.mm(x1, x2.T) ** 2) + torch.mm(x1, x2.T) + 1   # torch.mm effectue un produit matriciel
def relu(x):
...
class ThreeLayerNN:
    def __init__(self, p1, p2, d, lr, reg1, reg2, lambda_ridge, eps_init, FlagReinitialize=False, fraction_batch=0.1):
        ...
        # Move to device
        self.W1 = self.W1.to(device) #.to(device) sert à déplace un tenseur sur l'appareil de calcul 'device'
    ...
    def forward(self, x):
        ...
        h1 = relu(torch.mm(self.W1, x.t()) + self.b1).t()   # .t() effectue une transposition
    ...
    def train_layer1(self, x_batch, y_batch,  x_valid, y_valid, max_iter):
        ...
        for i in range(num_batches):
            indices = torch.randperm(x_batch.shape[0])[:mini_batch_size]    # torch.randperm(n) génère une liste des n premiers entiers permutée. Cette ligne permet de récupérer une liste d'indices aléatoires de taille batch_mini_size dans les données du batch
            x_mini_batch = x_batch[indices]
            y_mini_batch = y_batch[indices]
            output, h1, h2 = self.forward(x_mini_batch)
            # Backpropagation to compute dLoss/dW1
...
def cosine_similarity_nonlinear_joint_evolution(W1s, W2s, b1, b2, W1_stars, ws, FlagOverlapNonlin=False, sigma_star1=He_2):
    ...
    if FlagOverlapNonlin: 
        ...
        d = W1s[0].shape[1] # data dimension
        k = len(W1_stars)   # number of index of the MIGHT
        W1_star_tensor = torch.stack(W1_stars, dim=2).to(device).to(dtype)  # empilement dans la 3ème dimension de W1_stars de chaque index de MIGHT
        n_overlap = 1000    #number of samples to compute the mean of the overlaps
        X = torch.randn(n_overlap, d).to(device).to(dtype)  # generate n_overlap conctatenated over the first dimension of gaussian random samples.
        deff = W1_star_tensor.shape[1]  # première dimension effective 
        
        # Initialize overlap tensor of shape (T, p2, k)
        overlap_tensor = torch.zeros(T, W2s[0].shape[0], len(W1_stars)).to(device).to(dtype)    # Initialisation d'un tenseur zero de dimension (T, p2, k) pour l'overlap entre les neurones de la couche 2 (i.e. pour chaque pas de temps, pour chaque neurone de la couche cachée 2, pour chaque index de MIGHT)
        first_term = torch.einsum('tpd,nd->tnp', torch.stack(W1s), X)   # produit tensoriel qui résulte en un tenseur contenant la valeur des p1 neurones avant activation de la première couche pour chaque donnée X à chaque instant T
        bias_term = b1.squeeze(-1).unsqueeze(0).unsqueeze(1).expand(T, X.shape[0], b1.size(0)) # terme de biais aux bonnes dimensions
        h1s = relu(first_term + bias_term) # tenseur contenant la valeur des p1 neurones après activation de la première couche pour chaque donnée X à chaque instant T 
        
        
        # Compute U_star once since it's independent of W2
        W1_star_permuted = W1_star_tensor.permute(1, 0, 2)  # shape: (deff, d, k) # permutation des dimensions (axe0=axe1, axe1=axe0, axe2=axe2) pour que le produit tensoriel soit possible
        preactivations = sigma_star1(torch.einsum('dik,ni->dnk', W1_star_permuted, X))  # shape: (deff, n_overlap, k) # tenseur maître contenant la valeur des neurones de la première couche cachée avant activation pour chaque donné X pour chaque index
        ws = torch.stack(ws).to(device).to(dtype)
        ws = ws.view(k, deff) # shape: (k, deff)        # ws est le vecteur ? représentant a*T.P3()?
        U_star = (1/deff**.5) * torch.einsum('kd,dnk->nk', ws, preactivations)  # shape: (n_overlap, k) # renvoie la valeur des k index de MIGHT pour chaque donnée X
        ...
        numerator = torch.einsum('tnp,nk->tpk', U, U_star)  # shape: (T, p2, k) # tenseur contenant la somme sur tous les échantillons du produit du p-ième neurone p2 avec le k_ième index du MIGHT
        U_norm = torch.norm(U, dim=1)  # shape: (T, p2) # matrice contenant la norme de chaque vecteur du p-ième neurone sur toutes les données X en fonction du temps T
